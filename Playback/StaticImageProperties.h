...
namespace sip
{
	...
	获得相机深度模式对应范围
	参考：
	https://docs.microsoft.com/zh-cn/azure/kinect-dk/hardware-specification#depth-camera-supported-operating-modes
	*/
	inline std::pair<uint16_t, uint16_t> GetDepthModeRange(const k4a_depth_mode_t depthMode)
	{
		...
		case K4A_DEPTH_MODE_NFOV_2X2BINNED:
			return { (uint16_t)500, (uint16_t)5460 };
		case K4A_DEPTH_MODE_NFOV_UNBINNED:
			return { (uint16_t)500, (uint16_t)3860 };
		case K4A_DEPTH_MODE_WFOV_2X2BINNED:
			return { (uint16_t)250, (uint16_t)2880 };
		case K4A_DEPTH_MODE_WFOV_UNBINNED:
			return { (uint16_t)250, (uint16_t)2210 };
		default:
